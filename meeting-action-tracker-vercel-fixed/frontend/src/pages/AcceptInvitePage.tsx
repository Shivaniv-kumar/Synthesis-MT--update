...
export default function AcceptInvitePage(): React.JSX.Element {
  ...
  async function onSubmit(data: FormData): Promise<void> {
    setApiError(null)
    if (!token) {
      setApiError('Invitation token is missing. Please use the link from your email.')
      return
    }
    try {
      await acceptInvite(token, data.new_password)
      setSuccess(true)
      setTimeout(() => void navigate('/login', { replace: true }), 3000)
    } catch (err: unknown) {
      const detail =
        (err as { response?: { data?: { detail?: string } } })?.response?.data?.detail
      setApiError(detail ?? 'This invitation link is invalid or has expired. Please ask your admin to resend it.')
    }
  }
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4">
      <div className="w-full max-w-md">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="mt-1 text-sm text-primary-500">Set up your account</p>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
          {success ? (
            <div className="text-center">
              ...
              <h2 className="text-base font-semibold text-primary-900">Account ready</h2>
            ...
            <>
              ...
              <p className="mb-5 text-sm text-primary-600">
                Welcome to Synthesis. Choose a password to activate your account.
              </p>
